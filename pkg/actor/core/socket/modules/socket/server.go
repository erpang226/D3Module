...
func (m *Server) processModuleMessage(conn wrapper.Conn, message *model.Message) error {
	...
	case common.OperationTypeModule:
		...
		actorContext.AddModule(add)
		actorContext.AddModuleGroup(moduleName, moduleGroup)
		resp := message.NewRespByMessage(message, core.GetModuleExchange())
		actorContext.SendResp(*resp)
	}
	return nil
}
...
func (m *Server) handleServerMessage(conn wrapper.Conn) error {
	...
	// klogGER.Infof("receive  message: %+v", message)
	...
	// klogGER.Infof("server: %+v", message)
	actorContext.Send(message.GetDestination(), message)
	return nil
}
